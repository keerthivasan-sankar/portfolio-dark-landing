  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex justify-center pt-4 md:pt-6 px-4">
      <motion.div
        ...
        animate={{
          boxShadow: scrolled ? '0 4px 20px rgba(0,0,0,0.15)' : '0 0 0 rgba(0,0,0,0)',
        }}
        transition={{ duration: 0.3 }}
      >
        {/* Logo */}
        <div className="flex items-center justify-center w-9 h-9 rounded-full bg-bg cursor-pointer hover:scale-110 transition-transform duration-300 relative">
          <div className="absolute inset-0 rounded-full accent-gradient opacity-80" style={{ padding: '1.5px' }}>
        ...
        <nav className="flex items-center gap-0.5">
          {navLinks.map((link) => (
            <button
              key={link.id}
              onClick={() => onNavigate(link.id === 'resume' ? 'contact' : link.id)}
              className={`
